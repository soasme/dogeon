...
def errmsg(msg, doc, pos, end=None):
    # Note that this function is called from _DSON
...
def DSONObject(s_and_end, encoding, strict, scan_once, object_hook,
    ...
    if nextchar != '"':
        ...
        if nextchar == 'w' and s[end: end + 3] == 'wow':
    ...
    while True:
        ...
        # the DSON key separator is "is " or just "is".

        if s[end:end + 2] != 'is':
            end = _w(s, end).end()
            if s[end:end + 2] != 'is':
                raise ValueError(errmsg("Expecting 'is' delimiter", s, end))
        end += 2
...
def DSONArray(s_and_end, scan_once, _w=WHITESPACE.match, _ws=WHITESPACE_STR):
    ...
    if nextchar == 'm' and s[end: end + 4] == 'many':
        return values, end + 4
    ...
    while True:
        ...
        if nextchar == 'm' and s[end: end + 4] == 'many':
            ...
            break
        if nextchar == 'a' and s[end: end + 3] == 'and':
            end += 3
        elif nextchar == 'a' and s[end: end + 4] == 'also':
            end += 4
        else:
            end += 1
            raise ValueError(errmsg("Expecting 'and|also' delimiter", s, end))
        try:
            if s[end] in _ws:
                end += 1
                if s[end] in _ws:
